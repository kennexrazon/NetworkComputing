...
def trending_alertgen(trending_alert, monitoring, lgd, window, config):
    ...
    if len(palert) != 0:
        ...
        for i in palert.index:
            try:
                palert.loc[palert.index == i].to_sql(name = 'node_level_alert_noFilter', con = engine, if_exists = 'append', schema = q.Namedb, index = False)
            except:
    ...
    if len(palert) != 0:
        for i in palert['id'].values:
            query = "SELECT * FROM senslopedb.node_level_alert_noFilter WHERE site = '%s' and timestamp >= '%s' and id = %s" %(monitoring.colprops.name, endTS-timedelta(hours=3), i)
